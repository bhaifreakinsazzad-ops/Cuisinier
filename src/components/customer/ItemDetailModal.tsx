...
export function ItemDetailModal({ item, onClose }: ItemDetailModalProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] flex items-end justify-center" onClick={onClose}>
      ...
      >
        <button onClick={onClose} className="absolute right-4 top-4 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-black/50 text-white/70 transition-colors hover:text-white">
          <X size={16} />
        </button>
        ...
        <div className="max-h-[90vh] overflow-y-auto">
          ...
          <div className="relative -mt-8 px-5 pb-8">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {item.tags.map((tag) => (
                <GlowBadge key={tag} variant="orange">
                  {tag}
                </GlowBadge>
              ))}
              {!item.available && (
                <GlowBadge variant="red">
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-center justify-between rounded-xl border p-3 transition-all ${
                        selected ? 'border-orange-500/40 bg-orange-500/15' : 'border-white/10 bg-white/5 hover:bg-white/10'
                      }`}
                    >
                      <span className={`text-sm ${selected ? 'text-orange-400' : 'text-white/80'}`}>{addon.name}</span>
                      <span className={`text-sm font-medium ${selected ? 'text-orange-400' : 'text-white/50'}`}>+{formatCurrency(addon.price)}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => setQuantity((current) => Math.max(1, current - 1))} className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white transition-colors hover:bg-white/20">
                  <Minus size={16} />
                </button>
                <span className="w-8 text-center text-lg font-bold text-white">{quantity}</span>
                <button onClick={() => setQuantity((current) => current + 1)} className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white transition-colors hover:bg-white/20">
                  <Plus size={16} />
                </button>
              </div>
            </div>
            ...
            <motion.button
              ...
              className={`mt-6 flex w-full items-center justify-center gap-2 rounded-2xl py-4 text-base font-bold transition-all ${
                added ? 'bg-green-500 text-black' : 'bg-orange-500 text-black shadow-[0_0_30px_rgba(255,122,0,0.3)] hover:bg-orange-600'
              } disabled:cursor-not-allowed disabled:opacity-45`}
            >
              {added ? 'Added to Cart!' : <>
                <ShoppingCart size={18} />
                {item.available ? `Add to Cart - ${formatCurrency(itemTotal)}` : 'Currently unavailable'}
              </>}
            </motion.button>
          </div>
        </div>
