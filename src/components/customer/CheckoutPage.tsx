...
import { formatCurrency, isLikelyBangladeshMobile } from '@/lib/utils';
...
export function CheckoutPage() {
    ...
    return (
      <div className="flex min-h-[100dvh] items-center justify-center bg-[#080808] px-5">
        <div className="text-center">
          ...
          <button onClick={() => navigate('/menu')} className="mt-4 rounded-xl bg-orange-500 px-6 py-3 font-bold text-black">
            Browse Menu
          </button>
        </div>
      </div>
  ...
  const validate = () => {
    ...
    } else if (!isLikelyBangladeshMobile(phone)) {
      nextErrors.phone = 'Use a valid Bangladesh mobile number.';
    }
    ...
    if (manualPaymentSelected) {
      if (!isLikelyBangladeshMobile(paymentDestination)) {
        nextErrors.paymentMethod = 'Manual payment is temporarily unavailable. Please use Cash on Delivery.';
      }

      if (!senderNumber.trim()) {
        nextErrors.senderNumber = 'Sender number is required for manual payment.';
      } else if (!isLikelyBangladeshMobile(senderNumber)) {
        nextErrors.senderNumber = 'Use the valid sender number used for payment.';
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const order = await orderRepository.create({
        ...
        transactionId: paymentMethod === 'cod' ? undefined : transactionId.trim(),
        status: 'placed',
        createdAt: now,
        updatedAt: now,
      });
  ...
  const inputClass = (field: string) =>
    `w-full rounded-xl border px-4 text-sm text-white placeholder:text-white/30 focus:outline-none transition-colors ${
      errors[field] ? 'border-red-500/50' : 'border-white/10 focus:border-orange-500/50'
    } bg-white/5`;
    ...
    <div className="relative min-h-[100dvh] bg-[#080808]">
      ...
      <div className="relative z-10 mx-auto max-w-xl px-5 py-6 pb-40">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <button onClick={() => navigate('/cart')} className="mb-4 text-sm text-white/50 transition-colors hover:text-white/80">
            ← Back to Cart
          </button>
                ...
                return (
                  <div key={`${entry.menuItem.id}-${index}`} className="text-sm">
                    ...
                    {entry.addons.length > 0 && (
                      <p className="text-xs text-white/35">Add-ons: {entry.addons.map((addon) => addon.name).join(', ')}</p>
                    )}
                    {entry.note && <p className="text-xs italic text-white/30">Note: {entry.note}</p>}
                  </div>
                );
              })}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          ...
          <div className="space-y-3">
            <div>
              <input value={name} onChange={(event) => { setName(event.target.value); setErrors((current) => ({ ...current, name: '' })); }} placeholder="Customer Name *" className={`${inputClass('name')} h-12`} />
              {errors.name && <p className="mt-1 text-xs text-red-400">{errors.name}</p>}
            </div>
            <div>
              <input value={phone} onChange={(event) => { setPhone(event.target.value); setErrors((current) => ({ ...current, phone: '' })); }} placeholder="Phone Number *" className={`${inputClass('phone')} h-12`} />
              {errors.phone && <p className="mt-1 text-xs text-red-400">{errors.phone}</p>}
            </div>
            <div>
              <textarea value={address} onChange={(event) => { setAddress(event.target.value); setErrors((current) => ({ ...current, address: '' })); }} rows={3} placeholder="Delivery Address *" className={`${inputClass('address')} resize-none py-3`} />
              {errors.address && <p className="mt-1 text-xs text-red-400">{errors.address}</p>}
            </div>
            <div>
              <select value={area} onChange={(event) => { setArea(event.target.value); setErrors((current) => ({ ...current, area: '' })); }} className={`${inputClass('area')} h-12 appearance-none`}>
            ...
            <div>
              <textarea value={specialNote} onChange={(event) => setSpecialNote(event.target.value)} rows={2} placeholder="Special note (optional)" className={`${inputClass('specialNote')} resize-none py-3`} />
            </div>
            <p className="text-[11px] text-white/35">Night: Dhaka service. Daytime: Dhanmondi and nearby areas.</p>
          </div>
        </motion.div>
            ...
            {[
              ...
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-xl border p-3.5 text-left transition-all ${
                  paymentMethod === option.value ? 'border-orange-500/40 bg-orange-500/15' : 'border-white/10 bg-white/5 hover:bg-white/10'
                }`}
              >
                <option.icon size={20} className={paymentMethod === option.value ? 'text-orange-400' : 'text-white/50'} />
                <div className="flex-1">
                  <span className={`block text-sm font-medium ${paymentMethod === option.value ? 'text-orange-400' : 'text-white/80'}`}>{option.label}</span>
                  <span className="text-xs text-white/35">{option.caption}</span>
                </div>
                {paymentMethod === option.value && <span className="ml-auto h-5 w-5 rounded-full bg-orange-500" />}
              </button>
            ))}
          </div>
        ...
        {manualPaymentSelected && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-4">
            <GlassCard>
              ...
              <div>
                <input value={senderNumber} onChange={(event) => { setSenderNumber(event.target.value); setErrors((current) => ({ ...current, senderNumber: '' })); }} placeholder={`Sender Number (${paymentMethod === 'bkash' ? 'bKash' : 'Nagad'}) *`} className={`${inputClass('senderNumber')} h-12`} />
                {errors.senderNumber && <p className="mt-1 text-xs text-red-400">{errors.senderNumber}</p>}
              </div>
              <div className="mt-3">
                <input value={transactionId} onChange={(event) => { setTransactionId(event.target.value); setErrors((current) => ({ ...current, transactionId: '' })); }} placeholder="Transaction ID *" className={`${inputClass('transactionId')} h-12`} />
        ...
        {errors.submit && <p className="mt-4 text-center text-xs text-red-400">{errors.submit}</p>}
        ...
        >
          {submitting ? 'Placing Order...' : <>Place Order - {formatCurrency(totals.total)} <ArrowRight size={18} /></>}
        </motion.button>

        <p className="mt-3 text-center text-[11px] text-white/20">
          Orders are confirmed immediately after submission. No customer-side cancel button in v1.
        </p>
      </div>
    </div>
  );
